...
class Alpha(object):
    ...
    def set_basic_data(
        ...
        zz500_close = read_market(close=1, index='000802', freq=cls.freq)

        cls.open = open.apply(pd.to_numeric, errors='coerce')
        cls.close = close.apply(pd.to_numeric, errors='coerce')
        zz500_close = zz500_close.apply(pd.to_numeric, errors='coerce')
        cls.index_close = zz500_close
        cls.market_ret = zz500_close / zz500_close.shift(1) - 1
    ...
    @lru_cache(maxsize=None)
    def get_rets(cls):
        cls.rets = (cls.close - cls.open) / cls.open
